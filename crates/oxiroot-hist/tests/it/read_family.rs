...
use oxiroot_hist::{Hist1D, Hist2D, Hist3D, Profile1D, ReadRoot};
...
#[test]
fn reads_th1f() {
    let h = Hist1D::read_root(&open("th1f_uncompressed.root"), "h1f").expect("TH1F");
...
#[test]
fn reads_th2f() {
    let h = Hist2D::read_root(&open("th2f_uncompressed.root"), "h2f").expect("TH2F");
...
#[test]
fn reads_th3d() {
    let h = Hist3D::read_root(&open("th3d_uncompressed.root"), "h3").expect("TH3D");
...
fn th3f_matches_th3d_values() {
    let d = Hist3D::read_root(&open("th3d_uncompressed.root"), "h3").unwrap();
    let f = Hist3D::read_root(&open("th3f_uncompressed.root"), "h3").unwrap();
...
#[test]
fn reads_tprofile() {
    let p = Profile1D::read_root(&open("tprofile_uncompressed.root"), "p").expect("TProfile");
    assert_eq!(p.name, "p");
