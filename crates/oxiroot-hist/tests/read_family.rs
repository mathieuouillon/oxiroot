...
use oxiroot_hist::{ReadRoot, TProfile, TH1, TH2, TH3};
...
#[test]
fn reads_th1f() {
    let h = TH1::read_root(&open("th1f_uncompressed.root"), "h1f").expect("TH1F");
...
#[test]
fn reads_th2f() {
    let h = TH2::read_root(&open("th2f_uncompressed.root"), "h2f").expect("TH2F");
...
#[test]
fn reads_th3d() {
    let h = TH3::read_root(&open("th3d_uncompressed.root"), "h3").expect("TH3D");
...
fn th3f_matches_th3d_values() {
    let d = TH3::read_root(&open("th3d_uncompressed.root"), "h3").unwrap();
    let f = TH3::read_root(&open("th3f_uncompressed.root"), "h3").unwrap();
...
#[test]
fn reads_tprofile() {
    let p = TProfile::read_root(&open("tprofile_uncompressed.root"), "p").expect("TProfile");
    assert_eq!(p.name, "p");
